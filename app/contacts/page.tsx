...
function RowActionsMenu({ isPrimary, onEdit, onDelete }: {
  ...
  useEffect(() => {
    function handle(e: MouseEvent) {
      if (btnRef.current && !btnRef.current.closest('[data-row-menu]')?.contains(e.target as Node)) {
        setOpen(false);
      }
    }
  ...
  const menu = (
    <div data-row-menu
      ...
      className="bg-white border border-[#e8e6df] rounded-xl shadow-xl py-1 w-44">
      {isPrimary && (
